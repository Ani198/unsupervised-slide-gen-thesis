...
class LsaSummarizer(BaseSummarizer):
    ...
    def __call__(self, document, topic_num, sent_per_topic):
        ...
        matrix = self._compute_tf_idf_matrix(matrix)
        #print(matrix)
        #print(matrix.shape)
        ...
        for idxs in v[:topic_num,:].argsort()[:,-sent_per_topic:]:
            keysentences = []
            for idx in idxs:
                keysentences.append(sentences[idx])
    ...
    def _compute_tf_idf_matrix(self, matrix, smooth=0.4):
        """
        ...
        for col in range(cols):
            sent_col = matrix[:, col]
            sentence_word_count[col] = len(sent_col[sent_col > 0])
        
        # number of sentences containing each word
        word_sent_count = {}
        for row in range(rows):
            word_row = matrix[row, :]
            word_sent_count[row] = len(word_row[word_row > 0])
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if sentence_word_count[col] != 0:
                    tf = matrix[row, col]/sentence_word_count[col]
                else:
                    tf = matrix[row, col]
                    
                if word_sent_count[row] != 0:
                    idf = math.log10(cols / word_sent_count[row])
                else:
                    idf = 1
